...
export default async function CityPage({ params }: { params: Promise<{ citySlug: string }> }) {
  ...
  const { data, error } = await safeListRecommendations({ city: citySlug, limit: 200 });
  ...
  return (
    <main className="page-shell" data-testid="city-page">
      ...
      <section className="rec-masonry">
        {data.recommendations.map((recommendation) => (
          <RecommendationCard key={recommendation.id} recommendation={recommendation} />
        ))}
      </section>

      {!data.recommendations.length ? <p className="notice">No recommendations found for this city.</p> : null}
    </main>
  );
}
